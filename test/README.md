...
`test-profiles.py` (offline) pins the per-model profiles (`tokenization/{qwen3_5,qwen3_6,qwen3_8,llama3_1}.py`): the stated strict-TITO chat markers, tool-call parser, and reasoning format per family, and — the load-bearing check — that a stream built from the profile's markers matches the template's own render of the same conversation.
...
`test-tokenizer-fingerprint.py` (offline) pins the tokenizer-identity fingerprint (training–rollout consistency): the cross-repo probe-corpus digest (the same constant is pinned in `harbor.slime_bridge`'s copy of the algorithm, so the two implementations cannot drift silently), algorithm determinism and sensitivity — with the chat template deliberately *outside* the strict fingerprint — the registry's per-profile caching, the `GET /tokenizer_fingerprint` surface (open, 404 naming the mapping for an unmapped model, 501 in relay mode), and the recorded pins of the bundled Qwen3.5, Qwen3.6, and Qwen3.8 tokenizers. Those pins are the deployment pin: changing the bundle or the profile markers is reward-adjacent, and updating the constants here is where it gets acknowledged.

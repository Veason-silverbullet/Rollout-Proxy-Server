...
async def test_layering() -> None:
    ...
    # The RL policy must neutralize every request-controlled distribution knob,
    # including zero-valued overrides. Conflicting YAML defaults must lose too.
    requested = {
        "temperature": 0.7, "top_p": 0.9, "top_k": 20, "min_p": 0.1,
        "repetition_penalty": 1.2, "presence_penalty": 0.5, "frequency_penalty": 0.5,
    }
    neutral = {
        "temperature": 1.0, "top_p": 1.0, "top_k": -1, "min_p": 0.0,
        "repetition_penalty": 1.0, "presence_penalty": 0.0, "frequency_penalty": 0.0,
    }
    provider = SamplingProvider(sampling_overrides=requested)
    provider.set_runtime_sampling_overrides(neutral)
    params = await one_turn(provider, "neutral-rl-policy", stop=["END"], **requested)
    check("all neutral RL overrides beat config and request", all(params[k] == v for k, v in neutral.items()))
    check("neutral RL policy preserves token limits and stops", params["max_tokens"] == 8 and params["stop"] == ["END"])

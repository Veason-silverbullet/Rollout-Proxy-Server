...
Appending only the header keeps that adaptation out of the template while
giving the engine the header-only prompt the checkpoint expects.  The bundled
``chat_template.jinja`` is otherwise the upstream release template with two
deliberate patches, both needed for strict-TITO delta renders (a delta of only
tool messages has no user query and starts with a tool message): the
``raise_exception('No user query found …')`` is disabled, and the tool-header
test reads ``loop.first or loop.previtem.role != "tool"`` (comments in the
template mark both).  The unpatched upstream copy sits beside it as
``chat_template-legacy.jinja``, identical to the template embedded in
``tokenizer_config.json``.  A *headerless* prompt is not an option either: the

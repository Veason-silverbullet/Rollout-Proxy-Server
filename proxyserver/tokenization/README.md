...
Qwen3.6 and Qwen3.8 use the same two delta-render patches in their respective `chat_template.jinja` files, with each original retained as `chat_template-legacy.jinja`. Qwen3.6 preserves its upstream assistant-history behavior; Qwen3.8 additionally preserves its reasoning-effort system prefix. Both profiles supply the same bare assistant header as Qwen3.5, and both bundle sampling defaults of temperature `1.0`, top-k `20`, and top-p `0.95`.

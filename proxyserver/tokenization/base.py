...
def _drop_think_blocks(text: str) -> str:
    ...
    An unterminated opener consumes the remaining text: even complete tool
    calls inside it are reasoning, not actions. The raw completion remains
    available in the session record.
    """
    parts: list[str] = []
    pos = 0
    while True:
...
class ThinkReasoning:
    ...
    leading reasoning *and* every later ``<think>...</think>`` span, including
    unfinished blocks —
    ...
    def strip(self, text: str) -> str:
        ...
        if not sep:
            return _drop_think_blocks(text)

...
class InferenceWorkerClient:
    ...
    def __init__(
        ...
        # In-flight inference and cleanup tasks. Tracked on the client, NOT per
        ...
        self._closing_sessions: dict[str, asyncio.Task] = {}
    ...
    async def _handle_request(self, request: dict[str, Any]) -> None:
        ...
        if request.get("session_id") in self._closing_sessions:
            await self._deliver_response(_response_message(
                request_id,
                _error_response("Session cleanup is in progress", error_type="invalid_request"),
            ))
            return
    ...
    def _start_request(self, request: dict[str, Any]) -> None:
        ...
        def done(finished: asyncio.Task) -> None:
            ...
            if not finished.cancelled() and finished.exception() is not None:
                logger.error("Inference request failed: %s", finished.exception())

        task.add_done_callback(done)

    async def _close_session(self, session_id: str) -> None:
        """Cancel and await all work for a stream before releasing provider state."""
    ...
    def _start_session_close(self, data: dict[str, Any], ws: Any) -> None:
        """Drain in the background so other streams can still receive work."""
        session_id = data["session_id"]
        cleanup = self._closing_sessions.get(session_id)
        if cleanup is None:
            cleanup = asyncio.create_task(self._close_session(session_id))
            self._closing_sessions[session_id] = cleanup
            self._tasks.add(cleanup)

            def done(finished: asyncio.Task) -> None:
                self._closing_sessions.pop(session_id, None)
                self._reap_control_task(finished)

            cleanup.add_done_callback(done)

        async def acknowledge() -> None:
            await asyncio.shield(cleanup)
            if data.get("close_id"):
        ...
        self._tasks.add(ack)
        ack.add_done_callback(self._reap_control_task)

    def _reap_control_task(self, task: asyncio.Task) -> None:
        self._tasks.discard(task)
        if not task.cancelled() and task.exception() is not None:
            logger.warning("Session cleanup or acknowledgement failed: %s", task.exception())
            ...
            try:
                ...
                async for raw_msg in ws:
                    ...
                    if msg_type == "completion_request":
                        ...
                        # Acknowledge only after inference has stopped and
                        # the model pin and strict-TITO stream are released.
                        session_id = data.get("session_id")
                        if session_id:
                            self._start_session_close(data, ws)

...
per-session state such as strict-TITO token streams)::
    ...
    {"type": "session_closed", "session_id": "<stream-id>", "close_id": "<uuid>"}

The worker cancels and awaits the stream's inference tasks, releases its
state, then acknowledges with ``session_closed_ack`` carrying the same
``session_id`` and ``close_id``. DELETE succeeds only after acknowledgement.
...
class InferenceRelay:
    ...
    def __init__(self, request_timeout: float = 600.0) -> None:
        ...
        # close_id -> (worker_id, stream_id, acknowledgement future)
        self._close_waiters: dict[str, tuple[str, str, asyncio.Future]] = {}
    ...
    def unregister_worker(self, worker_id: str, ws: WebSocket) -> None:
        ...
        for wid, _, future in self._close_waiters.values():
            if wid == worker_id and not future.done():
                future.set_exception(RuntimeError(f"Worker {worker_id} disconnected during session cleanup"))
    ...
    def shutdown(self) -> None:
        ...
        for request_id in list(self._pending):
            ...
            if pending is not None and not pending.future.done():
                pending.future.set_exception(RuntimeError("Proxy is shutting down; request abandoned"))

    async def release_session(self, session_id: str) -> None:
        """Wait for worker cancellation before dropping a stream's binding.

        A missing worker, missing acknowledgement, or failed send is a cleanup
        failure. Keep the binding so deletion can be retried after reconnect.
        """
        bound = self._session_workers.get(session_id)
        workers = {bound} if bound is not None else set(self._workers)
        waiters = []
        try:
            async with asyncio.timeout(5.0):
                for wid in workers:
                    ...
                    if ws is None:
                        raise RuntimeError(f"Worker {wid} is disconnected; session cleanup is unconfirmed")
                    close_id = uuid4().hex
                    future = asyncio.get_running_loop().create_future()
                    self._close_waiters[close_id] = (wid, session_id, future)
                    waiters.append((close_id, future))
                    await ws.send_json({
            ...
            raise RuntimeError(f"Timed out waiting for session {session_id} cleanup acknowledgement") from e
        finally:
            for close_id, future in waiters:
                self._close_waiters.pop(close_id, None)
                if not future.done():
                    future.cancel()
                elif not future.cancelled():
                    future.exception()
        self._session_workers.pop(session_id, None)
        for requests in (self._pending, self._expired):
            for request_id, pending in list(requests.items()):
                if pending.session_id == session_id:
                    requests.pop(request_id, None)
                    if not pending.future.done():
                        pending.future.set_exception(RuntimeError(f"Session {session_id} was deleted"))
    ...
    async def handle_worker_ws(self, ws: WebSocket) -> None:
        ...
        try:
            ...
            while True:
                ...
                elif msg_type == "session_closed_ack":
                    waiter = self._close_waiters.get(data.get("close_id"))
                    if waiter is not None:
                        wid, session_id, future = waiter
                        if wid == worker_id and session_id == data.get("session_id") and not future.done():

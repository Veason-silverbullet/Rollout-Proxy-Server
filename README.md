...
- **The rollout is deleted** — `DELETE /sessions/{id}` refuses new turns with 410, cancels queued requests and active inference, and waits for cancellation cleanup before releasing each agent's stream. Detached local turns are included. Relay workers acknowledge only after their inference tasks have stopped and their provider state is released. A disconnected worker or an acknowledgement missing after 5 seconds per stream produces 503; the binding is retained so DELETE can be retried. Concurrent DELETEs share cleanup, which continues if the caller disconnects.
